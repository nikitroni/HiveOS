...
Все терминалы общаются по rednet небольшим протоколом «запрос/ответ», так что их можно расставить где угодно и перестроить пасеку правкой конфига на центральном терминале, а не редактированием скриптов.
...
Схема сети, включая передачу пчёл из улья в лабораторию, показана ниже и подробно разобрана в [Документации систем](docs/systems.ru.md).
...
Каждому терминалу нужен **проводной модем (Wired Modem) + Networking Cable, подключённый к каждому терминалу** (рекомендуется продвинутый компьютер). Список модов может расширяться по мере адаптации под новые сборки.

## 🚀 Установка

1. Создай и поставь **три компьютера** с мониторами: для HeartOS, BeeOS и LabOS.
2. Подключи **проводной модем (Wired Modem) + Networking Cable** к каждому терминалу и размести мониторы там, где нужно.
...
Пошаговая инструкция с подключением кабелей и мониторов — в [Инструкции по установке](docs/setup.ru.md).
...
Подробности всех трёх систем — в [Документации систем](docs/systems.ru.md).
...
Подробный план — в [Плане развития](docs/roadmap.ru.md).
