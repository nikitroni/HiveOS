...
Схема сети, включая передачу пчёл из улья в лабораторию, показана ниже и подробно разобрана в [`docs/systems.md`](docs/systems.md).
...
- **ComputerCraft: Tweaked** — три компьютера и мониторы.
- **Advanced Peripherals** — Block Reader и чат-бокс.
- **Productive Bees** — ульи, камера разведения, инкубатор, индексатор генов.
- **Just Dire Things** — кликер для автоматизации разведения.
- **Modular Routers** — маршрутизация предметов вокруг лаборатории и ульев.
- **Applied Energistics 2 (AE2)** — интерфейс ресурсов, подающий медовые лакомства.
- **SuperFactoryManager (SFM)** — «клей» автоматизации для больших пасек.
- **Ender IO** — кондуиты, энергия и машины вокруг пасеки.
...
Пошаговая инструкция с подключением кабелей и мониторов — в [`docs/setup.md`](docs/setup.md).
...
Подробности всех трёх систем — в [`docs/systems.md`](docs/systems.md).

## 📸 Скриншоты

> Ниже заглушки. Что именно должно быть на каждом изображении и как заменить URL на реальные снимки — в [`docs/screenshots/README.md`](docs/screenshots/README.md).
